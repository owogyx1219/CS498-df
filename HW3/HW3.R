...
reconstructAwithB <- function(A_category, A_means, A_pc, B_pc, index)
{
    sum_pc <- 0.0
    imageA <- A_category[[imageIndex]]
    diffA <- imageA - A_means
    
    for(pc_index in 1:20)n
    {
      #uA_pc <- A_pc[ , pc_index]
      uB_pc <- B_pc[ , pc_index]
      sum_pc <- sum_pc + t(uB_pc) %*% diffA %*% uB_pc
    }
...
for(i in 0:9)
{
  ...
  for(j in 0:9)
  {
      ...
      for(imageIndex in 1:6000)
      {
        ...
        recAImage <- reconstructAwithB(A_category, A_means, A_pc, B_pc, imageIndex)
        recBImage <- reconstructAwithB(B_category, B_means, B_pc, A_pc, imageIndex)
...
colnames(distance_matrix) <- c('airplane','automobile','bird','cat','deer','dog','frog','horse','ship','truck')
print(distance_matrix)
fit <- cmdscale(t(as.matrix(distance_matrix)),eig=TRUE, k=2)
x <- fit$points[,1]
y <- fit$points[,2]
plot(x, y,xlab ='', ylab = '',xlim=c(-1600, 1600), ylim=c(-800, 800), main="Map of The Means of Each Categories",	type="n")
text(x, y, labels = row.names(distance_matrix), cex=.7)