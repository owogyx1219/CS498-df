...
```{r}
train_list <- vecQuantization(trainData, trainSize)
train_labels <- getLabels(trainData, trainSize)

temp <- as.vector(train_list[[1]])
for(i in 2:length(train_list))
{
  temp <- c(temp, train_list[[i]])
}
...
test_list <- vecQuantization(testData, testSize)
test_labels <- getLabels(testData, testSize)
tempt <- as.vector(test_list[[1]])
for(i in 2:length(test_list))
{
  tempt <- c(tempt, test_list[[i]])
}
...
#fit = randomForest(hist_list$V15 ~., data = hist_list)
