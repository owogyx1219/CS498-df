...
trainSize <- floor(0.75*length(class_list[[1]]))
...
cutData <- function(trainData){
  ...
  cuttedData <- matrix(cuttedData, ncol = 3*cutSize + 1, byrow = TRUE)
  return (cuttedData)
}
cuttedTrainDataWithLabels <- cutData(trainData)
cuttedTestDataWithLabels <- cutData(testData)

trainDataLabels <- cuttedTrainDataWithLabels[, 3*cutSize + 1]
testDataLabels <- cuttedTestDataWithLabels[, 3*cutSize + 1]

cuttedTrainData <- cuttedTrainDataWithLabels[, 1:3*cutSize]
cuttedTestData <- cuttedTestDataWithLabels[, 1:3*cutSize]
...
```{r}
cluster_nums <- 40
kmeans_14 = kmeans(cuttedTrainData,centers = cluster_nums)
...
vecQuantization <- function(trainData, trainSize)
{
  ...
  for(index in 1:trainSize)
  {
    ...
    cuttedOneTrainData <- cuttedOneTrainDataWithLabels[,1:3*cutSize]
  ...
  for(i in 1:length(cuttedDataPoints))
  {
    ...
    hist_summary <- rep(0,cluster_nums)
...
getLabels <- function(trainData, trainSize)
{
  ...
  for(index in 1:trainSize)
  {
    ...
    currLabel <- cuttedOneTrainDataWithLabels[,3*cutSize + 1]
...
train_mat <- matrix(temp, ncol = cluster_nums, byrow = TRUE)
...
test_mat <- matrix(tempt, ncol = cluster_nums, byrow = TRUE)
#print(head(test_mat))
...
```{r}
train_mat_with_labels = data.frame(matrix(unlist(train_mat_with_labels),ncol = cluster_nums+1,byrow = FALSE))
print(head(train_mat_with_labels))
```

```{r}
test_mat = data.frame(matrix(unlist(test_mat),ncol = cluster_nums,byrow = FALSE))
fit = randomForest(train_mat_with_labels$X41 ~., data = train_mat_with_labels)
prediction = predict(fit,test_mat, predict.all = TRUE)

predicted = prediction$aggregate

matrix_value = confusionMatrix(table(factor(round(predicted,0),levels=1:cluster_nums),factor(test_labels,levels=1:cluster_nums)))
matrix_value
```
